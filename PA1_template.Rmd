...
I will use the following libraries in the analysis 
...
The working directory has to be proprly setted up. I will suppose that the required project has been cloned in the home 
...
I add a column with a properly formatted data too
```{r, echo=TRUE}
...
I calculate then the mean and the median total number of steps ignoring the days with missing values
...
and the interval with the max mean number of steps easaly calculated
```{r, echo=TRUE}
...
Having full days of missing values I will use the mean values for each interval calculated across the other days. These values have been previously calculated and recorded in the data frame **mean_time_series**. I'm going to save the new dataset in the variable **new_dataset**
...
To plot the histogram of the total number of steps across the days I have to calculate these values on the new data too
...
The column working_week contains a proper label to summarize data in the requested way
```{r, echo=TRUE}
